...
func (g *Generator) Generate(intf *Interface, pkgName string, outputPackagePath string) error {
...
func (g *Generator) generateFakeImpl(intf *Interface, outputPackagePath string) error {
	g.p("")
	g.p("type Fake%s struct {", intf.Name)

	for _, m := range intf.Methods {
		f := FuncType{m.Args, m.Results}
...
func formalArgsString(params []*Parameter, pt PackageTable) string {
...
func actualArgsString(params []*Parameter) string {
	args := make([]string, len(params))
	for i, p := range params {
		name := p.Name
		if name == "" {
...
func resultsString(params []*Parameter, pt PackageTable) string {
